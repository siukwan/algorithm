...
2.设起始点到环入口的距离为start
3.当两个指针相遇时，设相遇的地方为firstMeet，设环入口到指针相遇地点的距离为common
...
8.所以后面令一个指针从头开头走，另外一个指针从相遇点开始走，两个指针每次都走一步
9.假设他们最终相遇在firstMeet，那么相遇前它们都走了common的距离，所以它们重合的地方距离firstMeet相差common，即为环的入口。
